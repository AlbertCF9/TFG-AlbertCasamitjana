...
for i in ["DDL"]:
    ...
    c.SaveAs(f"fit_Omegac_M_2016_{i}.png")
    ...
    np_variables = df.AsNumpy(columns=variables_needed)

    df_variables = pd.DataFrame(np_variables)
    ...
    df_variables.to_csv(f"sweights_omegac_{i}_2016.csv", index=False)